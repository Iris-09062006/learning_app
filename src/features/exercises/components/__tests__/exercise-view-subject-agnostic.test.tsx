...
describe("ExerciseView subject-agnostic modalities", () => {
  it("renders a conceptual Exercise without a code block", () => {
    ...
    expect(screen.getByText("Tráº¯c nghiá»‡m â€¢ medium")).toBeInTheDocument();
  ...
  it("submits a short answer with the type-specific payload", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(JSON.stringify({
      success: true,
      data: { submissionId: 9, exerciseId: 302, isCorrect: true, feedback: "ÄÃºng", attemptNumber: 1, lessonProgress: { lessonId: 20, status: "completed", completionPercentage: 100 } },
    ...
    fireEvent.change(screen.getByLabelText("CÃ¢u tráº£ lá»i"), { target: { value: "  four values  " } });
    fireEvent.click(screen.getByRole("button", { name: "Ná»™p bÃ i" }));
  ...
  it("renders accessible ordering controls", () => {
    ...
    expect(screen.getByRole("button", { name: "ÄÆ°a First lÃªn" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "ÄÆ°a First xuá»‘ng" })).toBeEnabled();
  });
  ...
  it("renders matching prompts with answer choices from public option metadata", () => {
    ...
    expect(screen.getByLabelText("Individuals")).toHaveDisplayValue("Chá»n váº¿ phÃ¹ há»£p");
