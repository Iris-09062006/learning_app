...
- [x] **T025** [D2, P] `/courses/[courseId]` detail adopts the shared system
  - **Phase**: D · **Story**: US2 · **[P]**: yes (after T023) · **Prereq**: T023
...
- **T025 record**: detail aligned with the accepted Phase C system — visual-only, no functional change. Edits: `src/app/(main)/courses/[courseId]/page.tsx` (`<main>` → `bg-background py-8 sm:py-10 lg:py-12`; content into `PageContainer max-w-4xl pb-16 lg:pb-0` for mobile bottom-nav clearance; `cache()`/`notFound()`/metadata/T021 recommendation untouched) + `src/features/courses/components/course-detail-view.tsx` (header/summary card → `bg-surface border-border rounded-xl`, language tag + chapter ordinal chips → `bg-primary-soft text-primary`, raw `<button>` → shared `Button` with identical labels/loading/disabled semantics, legacy slate/indigo/slash-opacity classes → Stitch text tokens, added `data-testid="course-chapter-row"`) + token-guard `course-detail-view.test.tsx` (mirrors `course-card.test.tsx` guard regex for legacy palette + slash opacity). Browser verification (mock `:54321` + dev `:3001`): enroll behavior unchanged end-to-end — guest CTA shows error alert `role="alert"` (danger token `rgb(220,69,84)`), authenticated click flips CTA to "Bắt đầu học", enrolled click routes `/courses/1/roadmap`; responsive checks at 1440×900, 1024×768, 390×844 light+dark — all tokens derive correctly (primary `rgb(162,63,0)`, `bg-surface` `rgb(255,255,255)`/`rgb(40,34,29)`, chip `primary-soft`/`primary`), zero page horizontal overflow (`scrollWidth==clientWidth`), `pb-16` clears the 64px mobile bottom nav, keyboard focus ring visible on CTA/sidebar links (orange ring + surface halo); axe AA — light PASS (0 violations), dark PASS except a single pre-existing **out-of-scope** consumer: the T021-owned `CourseLearningRecommendation` CTA (`a.bg-blue-600` white-on-blue, 3.67:1, file untouched since `415f699`, only renders after enrollment). Gates rerun and PASS: `npm run lint` exit 0, `npm run typecheck` exit 0, focused `course-detail-view.test.tsx` 8/8, `course-pages.request-cache.test.tsx` 2/2, `npm run build` exit 0. `git diff --check` clean; temp axe/marker files removed from `public/` (dir empty again); evidence `tmp/t025-run/desktop-1440-{light,dark}-full.png`. T026+ untouched (checkboxes unchanged).
- [x] **T026** [D3, P] `/courses/[courseId]/roadmap` adopts the shared system
  - **Phase**: D · **Story**: US2 · **[P]**: yes (after T023) · **Prereq**: T023
...
- **T026 record**: roadmap aligned with the accepted Phase A–C system — visual-only, no functional change. Edits: `src/app/(main)/courses/[courseId]/roadmap/page.tsx` (`<main>` → `bg-background` tokens; content into `PageContainer max-w-4xl pb-16 lg:pb-0` for mobile bottom-nav clearance; layout only, no behavior touch) + `src/features/courses/components/course-roadmap-view.tsx` (progress fill `bg-indigo-600`→`bg-primary`, track `bg-slate-200/60`→`bg-surface-container` (A); trip-leg header card → `bg-surface border-border rounded-xl`, chapter header `bg-slate-50/50`→`bg-surface-subtle border-b border-border`, list rows `divide-y divide-border` + `border-b border-border` (C); success chip → `bg-success-soft text-success`, in-progress chip → `bg-info-soft text-info` (B); icons completed→`text-success`, in-progress→`text-primary`, locked→`text-text-muted`; legacy slate/indigo/slash-opacity utilities + hardcoded hex → Stitch tokens; focus ring visible (2px `outline-solid` `outline-primary` + `ring-4 ring-surface`); semantics/status mapping/lesson links/data-testid rows unchanged) + token-guard `course-roadmap-view.test.tsx` (now 14 tests: className + legacy-palette/slash-opacity guard regex mirroring T024/T025, chip/icon/row assertions). **Plus `src/app/globals.css`**: light-scheme `--success: #22a559`→`#15803d` (with T026 comment) — the T021-only warning/info darkening missed `--success`; this closes the WCAG AA failure on the success chip (4.5:1 on `--success-soft #e9f8ef`) at the token source so every `text-success`/`bg-success-soft` consumer inherits it. Browser verification (mock `:54321` + dev `:3001`, real route `/courses/1/roadmap`): enroll → CTA routes to roadmap; progress % reflects persisted lesson data (1/2 chapters → 50%; after starting lesson 101 and completing 1001 once, chips show success+info and zero stale locked icons); light + dark captured (light fill `rgb(162,63,0)`/`#a23f00`, dark fill settled `#ffb595`, success chip light `#15803d` on `#e9f8ef` + dark `#6fd392` on `#1a3323`, 900 ms post-`.dark` wait so the 500 ms theme transition fully settles); responsive 1440×900 and 390×844 light+dark — zero horizontal overflow, `pb-16` clears the fixed bottom nav (last card bottom 628 vs doc end 900); keyboard focus ring visible on lesson rows; axe AA light **0** + dark **0** violations. The roadmap route itself loads with **zero console errors and zero HTTP ≥400** (focused probe); the single global 403 `/api/courses/1/recommendations` appears only on the script's course-detail enroll step — the T021-owned AI-widget endpoint with no mock handler, pre-existing and out of scope (mirrors the T025 precedent). Gates PASS: `npm run lint` exit 0, `npm run typecheck` exit 0, focused roadmap 14/14 + badge 7/7 + course-card 7/7, `npm run build` exit 0; full suite 973 pass / 3 fail / 1 skip — the 3 fails are pre-existing `content-pipeline` SQL-migration snapshot assertions (`content-pipeline-migration`, `content-destination-migration`, `content-target-migration`: whitespace-sensitive `toContain` vs generated migration SQL; files untouched by this diff, reproduce at HEAD, out of scope). `git diff --check` clean; working tree limited to the 4 tracked files above (+ untracked spec/tmp scratch, nothing staged); nothing committed/pushed.

- [x] **T027** [D4, P] `/exercises/[exerciseId]` adopts the shared system
  - **Phase**: D · **Story**: US2 · **[P]**: yes (after T023) · **Prereq**: T023
...
- **T027 record**: exercise experience aligned with the accepted Phase A–C system — visual-only, no functional change. Edits: `src/app/(main)/exercises/[exerciseId]/page.tsx` (`<main>` slate → `bg-background`; content into `PageContainer max-w-5xl pb-16 lg:pb-0` for mobile bottom-nav clearance; layout only) + `src/features/exercises/components/exercise-view.tsx` (header card → `border-border bg-surface`; badge → `rounded-full bg-primary-container text-on-primary-container`; title/description/type → `text-text-primary/secondary/muted`; code snippet keeps `bg-code-background text-code-text`; option cards selected `border-primary bg-primary-soft` + radio dot `border-primary bg-primary`/`bg-on-primary`; focus rings → `ring-focus-ring ring-offset-background`; submit → shared `Button`; result panel success/danger `*-soft` + `*-strong` borders with semantic texts; error passthrough → `danger-soft`) + `src/features/exercises/components/fix-the-bug-drag-drop.tsx` (drop zone `bg-surface-subtle`/dragover `border-primary bg-primary-soft`; code chips keep `bg-code-background text-code-text`; "Gỡ bỏ" danger hover; option cards `border-border bg-surface` + `text-code-text`). Token-guard tests added in both exercise test files (legacy `slate|indigo|emerald|rose|white` palette guard; now 5 and 9 tests). Commits: `4bd3627` (+ this record). Browser verification (mock `:54321` + fresh dev `:3002`, real `/exercises/1001`): enroll → open exercise; computed-style token assertions light + dark match tokens 1:1 — badge `#f76b1c` on `#541d00` `rounded-full`, code pre `#151a2b`/`#e7ebf6` (dark `#14101d`), option idle `#ffffff`/`#e8e8e8`, selected `bg-primary-soft` + radio dot `#ffffff` on `#a23f00` (dark `#3b1a02` on `#ffb595`), submit `#a23f00` (dark `#ffb595`) via shared Button; wrong answer → danger panel `#fdecef`/`#dc4554` then retry with correct → success panel `#e9f8ef`/`#15803d` with same explanation text (behavior unchanged); keyboard focus ring `outline 3px solid #a23f00` (= `--focus-ring`, dark `#ffb595`); responsive 1440×900 + 390×844 light+dark — zero horizontal overflow, `pb-16` clears mobile bottom nav (light clearance 197px, dark 104px); axe AA **0/0** both themes. Console/HTTP: only the pre-existing T021-owned `403 /api/courses/1/recommendations` (AI-widget endpoint, no mock handler — identical to the T026 record); no exercise-screen console errors. Gates PASS: vitest exercise 14/14, `npm run lint` exit 0, `npm run typecheck` exit 0, `npm run build` exit 0, `git diff --check` clean. Scope audit: diff limited to the 5 source/test files; `src/app/globals.css`, backend/API/schema untouched; no fake Stitch features added (FR-042 visual patterns only).

- [x] **T028** [D5, P] `/profile` adopts the shared system
  - **Phase**: D · **Story**: US2 · **[P]**: yes (after T023) · **Prereq**: T023
...
- **T028 record**: `/profile` aligned with the accepted Phase A–C system — visual-only, no functional change. Edits: `src/app/(main)/profile/page.tsx` (`<main>` slate → `bg-background py-8 sm:py-10 lg:py-12`; content into `PageContainer max-w-4xl pb-16 lg:pb-0` for mobile bottom-nav clearance; layout only — `getOwnProfile`, UNAUTHENTICATED→`/login` redirect untouched) + `src/features/profile/components/profile-view.tsx` (`<main>` shell removed; back-link `text-indigo-600 dark:text-indigo-400` → `text-primary`; header → shared `PageHeader`; cards drop local `dark:border-slate-800 dark:bg-slate-900 dark:text-white` overrides → `Card` token defaults; dt labels `text-slate-500 dark:text-slate-400` → `text-text-muted`) + `src/features/profile/components/profile-form.tsx` (manual label/helper `<div>` → shared `Input` `label`/`helperText` with `aria-describedby` wiring preserved; message `text-red-600`/`text-emerald-600` → `text-danger`/`text-success`; username trim/min-length validation + PATCH flow untouched). Tests: `profile-form.test.tsx` (now 5 — adds error `role="alert"` `text-danger`, success `text-sm text-success`, visible helper-text accessibility, legacy-palette/`dark:`/slash-opacity token-guard mirroring T024–T027) + new `profile-view.test.tsx` (5 — h1/description render, back-link `/dashboard` + `text-primary`, account fields with `text-text-muted` dt, username form prefill, shared Card defaults `bg-surface border-border`, token-guard). Browser verification (mock Supabase `:54321` + dev `:3003`, `learner@example.com / Password123!`, real `/profile`): light tokens exact — `--primary #a23f00`, `--surface #ffffff`, `--background #fcf9f8`, `--border #e8e8e8`, `--focus-ring #a23f00`; body bg `rgb(252,249,248)`, card `#ffffff`/`#e8e8e8` 12px radius, dt `rgb(111,101,91)` (= text-muted), heading Be Vietnam Pro, back-link + save button `#a23f00` white text 8px radius (input 8px); keyboard focus on `#username` = `box-shadow rgb(252,249,248) 0 0 0 2px` (ring-surface) + `rgb(162,63,0) 0 0 0 4px` (= 4px `--focus-ring`); PATCH wiring confirmed live — captured `PATCH /api/profile` body `{"username":"Confirm new learner"}`; the mock has no PATCH handler so the real submit → 500 passthrough shows `role="alert"` `text-sm text-danger` "Unable to update profile."; success path verified via Playwright route-fulfillment (test-side, immediate restore) → `text-sm text-success` "Đã cập nhật username." + input value "Fulfilled Learner"; dark (`html.dark` + 900 ms settle wait for the 500 ms theme transition) tokens exact — `#ffb595`/`#28221d`/`#171210`/`#544940`, body bg settled `rgb(23,18,16)`, save `#ffb595` on `#3b1a02`; responsive 1440×900 + 390×844 light+dark — zero horizontal overflow (`scrollWidth==clientWidth`), `pb-16` clears the fixed bottom nav (mobile clearance light 32px / dark 37px); axe AA light + dark total=0, serious/critical=0. Console/HTTP: only the expected `500 http://localhost:3003/api/profile` from the mock's missing PATCH handler (environmental gap; success path covered by the route fulfillment) — no React errors. Gates PASS: `npm run lint` exit 0, `npm run typecheck` exit 0, focused profile tests 10/10 (form 5 + view 5), `npm run build` exit 0, `git diff --check` clean. Scope audit: diff limited to the 4 modified files + 1 new test file; `src/app/globals.css`, backend/API/schema and `.specify/feature.json` untouched; T029+ not started; nothing pushed/deployed.

- [x] **T029** [D6, P] `/dashboard` foundation adoption (B/C-bucket sweep, no dedicated redesign)
  - **Phase**: D · **Story**: US2 · **[P]**: yes (after T023) · **Prereq**: T023
...
- **T029 record**: `/dashboard` + adjacent AI surfaces aligned with the accepted Phase A–C system — foundation adoption only, no redesign. Edits: `src/app/(main)/dashboard/page.tsx` (`<main>` → `bg-background py-8 sm:py-10 lg:py-12`; content into `PageContainer pb-16 lg:pb-0` for mobile bottom-nav clearance; layout only — `getLearnerDashboard`, UNAUTHENTICATED→`/login` redirect, `force-dynamic`, metadata untouched) + `src/features/profile/components/dashboard-view.tsx` (shell `<main>` removed; header → shared `PageHeader` — same eyebrow/h1/description text; `Quản lý hồ sơ` outline button; metric cards → shared `Card` token defaults (`bg-surface border-border` 12px) + `text-text-muted` dt / `text-text-primary` dd; section headings `text-text-primary`; links `text-primary` + `hover:text-primary-hover`; empty state `border-border` dashed on `bg-surface`; course cards `rounded-xl border-border bg-surface`, status labels → semantic `text-success`/`text-text-muted`/`text-info` per status, progress track `bg-surface-container` + fill `bg-primary`, primary CTAs `bg-primary text-on-primary hover:bg-primary-hover`, outline `bg-surface border-border text-text-primary`; focus rings `ring-focus-ring ring-offset-background`; progressbar `aria-*` + labels/remarks unchanged) + `src/features/ai/components/learning-recommendation-card.tsx` (no-recommendation state `bg-surface-subtle border-border text-text-secondary`; card `bg-surface border-border`; heading/body/reason → `text-text-primary/secondary/muted`; **A-fix**: action link `a.bg-blue-600` (3.67:1 white-on-blue, the T025-recorded deferred dark finding, T021/T029-owned file) → `bg-primary text-on-primary` 12.2:1 light + `#ffb595`/`#3b1a02` dark with `ring-focus-ring` focus; status badge map → semantic pairs REVIEW_LESSON `warning-soft/warning`, COURSE_COMPLETED `success-soft/success`, RETRY_EXERCISE `danger-soft/danger`, NEXT_LESSON `info-soft/info`; behavior labels/testids untouched) + `src/features/ai/components/course-learning-recommendation.tsx` (loading skeleton `border-slate-200 bg-slate-100 dark:*` → `border-border bg-surface-subtle`; role/label untouched — resolves the T025 dark-mode skeleton note) + `src/features/ai/components/ai-explanation-view.tsx` (**D preserved**: trigger button, loading spinner + explanation panel use `ai`/`ai-soft`/`ai-hover`; indigo legacy text → readable text combos `text-text-primary/secondary/muted` on `ai-soft`; error → `danger`/`danger-soft`; divider `border-border`; fetch/handlers/aria untouched). Tests: `learning-recommendation-card.test.tsx` (now 7 — adds per-type semantic badge-map test, primary-CTA token test incl. no `blue-*`, legacy-palette/dark-prefix/slash-opacity token-guard) + `dashboard-view.test.tsx` (now 3 — adds semantic-token guard: track `bg-surface-container`, resume CTA `bg-primary text-on-primary`, zero legacy palette / `dark:` / slash-opacity utilities across rendered classes).
Browser verification (mock Supabase `:54321` + dev `:3000`, `learner@example.com / Password123!`, real routes, light + `html.dark` + 1–2.5 s settle for the 250 ms theme transition): `/dashboard` empty + populated light/dark — axe WCAG 2 A/AA total 0 / serious+critical 0 in all four states; light tokens exact `--primary #a23f00`, `--background #fcf9f8`, `--surface #ffffff`, `--border #e8e8e8`, `--text-primary #1b1c1c`, `--text-muted #6f655b`, `--focus-ring #a23f00`; body `rgb(252,249,248)`, metric Card `#ffffff`/`#e8e8e8` 12px, status "Đang học" `#2f66c2` (B info), progress track `bg-surface-container` + fill `bg-primary`, recommendation badge `info-soft #eaf2ff`/`info #2f66c2` 7.1:1, resume + "Học ngay" CTAs `#a23f00` white 8px (12.2:1); dark `#171210`/`#28221d`/`#544940`/`#f5efe9`/`#a99a90`, CTA `#ffb595` on `#3b1a02`, `info-soft #1c3049`/`info #8db9f2`; 1440×900 + 390×844 light/dark zero horizontal overflow (`scrollWidth==clientWidth`), bottom-nav content clearance via `pb-16`. `/courses/1` enrolled (recommendation CTA): light axe 0, dark axe 0 — the deferred `a.bg-blue-600` finding is closed; CTA measured `#a23f00`/white light and (settled) dark `#ffb595`. Exercise wrong-answer flow (`/exercises/1001`): trigger button `ai-soft #e6f8fc`/`text-primary` 15:1, border `ai #18a9c9`, 12px; AI request round-trips through mock provider + `POST /rest/v1/ai_explanations` ("Provider: openai-compatible" panel renders); explanation panel + loading + error states tokenized; axe dark settled 0/0. **Known residual (out of T029 scope, → Phase F)**: exercise result panel wrong-answer `text-danger #dc4554` on `bg-danger-soft #fdecef` = 3.66:1 small text — pre-existing T027 design (`exercise-view.tsx`, T027-file, untouched by T029 diff; T027 record itself documented this `#fdecef`/`#dc4554` pairing); transient dev-harness mid-transition captures (body still light while text tokens already dark) resolved by settled waits. Gates PASS: `npm run lint` exit 0, `npm run typecheck` exit 0, focused tests 9/9 across the two updated files (dashboard-view 3 + recommendation-card 6, all green), `npm run build` exit 0, `git diff --check` clean. Full-suite note (pre-existing, env-only, zero T029 files involved): 3 content-pipeline migration tests fail from Windows CRLF checkout of `supabase/migrations/*.sql` (documented 3-failure CRLF baseline since T005) and 1 `ai-provider` URL test fails when host `.env.local` sets `AI_PROVIDER_URL` (Gemini endpoint ≠ `api.openai.com`); `lesson-content-view` focus test flaked under load once then passed. Scope audit: diff limited to the 5 component/page files + 2 test files (7 total); `src/app/globals.css`, `(main)/layout.tsx` + shell, T024–T028 files, admin/moderation/pipeline (`content-pipeline`, `exercise-generation-form`), backend/API/schema/mock all untouched; nothing pushed/deployed. Evidence: `tmp/stitch-screens/t029-dashboard-{light-empty,light-populated,dark-populated}.png`, `tmp/t029-dev-server.log`.
...
- [x] **T030** [E1] Add-exercise AI generation flow restyle — `/moderation/lessons/[lessonId]/exercises/new`
...
- **T030 record**: add-exercise AI generation flow aligned with the accepted Phase A–C system — visual-only, no functional change. Edits: `src/app/(main)/moderation/lessons/[lessonId]/exercises/new/page.tsx` (`<main>` slate → `bg-background py-8 sm:py-10 lg:py-12`; content into `PageContainer pb-16 lg:pb-0` for mobile bottom-nav clearance; layout only — `getExerciseGenerationContext`, UNAUTHENTICATED→`/login`, FORBIDDEN→`/dashboard`, NOT_FOUND→`notFound()`, metadata untouched) + `src/features/ai/components/exercise-generation-form.tsx` (back link → `text-primary`; header → shared `PageHeader` (same h1/description text); "Mục tiêu chính thức" aside → `border-info bg-info-soft text-info`; error `role="alert"` → `border-danger bg-danger-soft text-danger`; result banner → `border-success bg-success-soft text-success` with "Mở draft" link; form card → shared `Card`; selects → shared `Select`, inputs → shared `Input` (labels/`datalist`/`required`/`maxLength` intact), submit → shared `Button` `size="lg"` orange primary with `isLoading`/`aria-busy` and `disabled` when objective empty; submit handler + `/api/ai/exercises/generate` payload + draft state unchanged) + `exercise-generation-form.test.tsx` (token-guard assertions added). Browser verification (mock `:54321` + dev `:3000`, `E2E Admin` session, real `/moderation/lessons/101/exercises/new`): axe WCAG 2 A/AA **0 violations / 26 passes** light and dark at 1024×768 (light `#fcf9f8`/`#1b1c1c`, dark `html.dark` `#171210`/`#f5efe9`); desktop/mobile light-dark + tablet light axe 0 in the earlier run. Interactive a11y (keyboard, 1024×768): Tab order logo → 8 nav links → logout → back link → Loại bài tập → Độ khó → Mục tiêu → Gợi ý → Sinh (all in viewport); every control shows `:focus-visible` `ring-2 ring-focus-ring` `#a23f00` (offset `--background`), plain links `2.67px solid #a23f00` outline; Enter inside "Gợi ý chủ đề" implicit-submits → `POST /api/ai/exercises/generate` **201** → banner "Draft … đang chờ moderation. Mở draft"; console 0 errors. Gates PASS: `npm run lint` exit 0, `npm run typecheck` exit 0, `npm run build` exit 0, focused vitest `exercise-generation-form.test.tsx` 9/9, `git diff --check` clean. Full-suite note (pre-existing, env-only, zero T030 files involved): 3 content-pipeline migration tests fail from Windows CRLF checkout of `supabase/migrations/*.sql` (documented 3-failure CRLF baseline since T005) and 1 `ai-provider` URL test fails when host `.env.local` sets `AI_PROVIDER_URL`. Commits: `f0b6640` (+ this record).

- [x] **T031** [E2] `/admin/content` content-pipeline visual alignment (component markup only)
...
- **T031 record**: `/admin/content` restyled to the shared Stitch primitives — visual-only, zero behavior/payload/label changes. Edits: `src/app/(main)/admin/content/page.tsx` (`<main>` `min-h-screen` + `bg-slate-50` → `bg-background py-8 sm:py-10 lg:py-12`; content into `PageContainer pb-16 lg:pb-0`; metadata/`assertAdminAccess`/`UNAUTHENTICATED→/login`/`FORBIDDEN→/dashboard` untouched) + `src/features/content-pipeline/components/content-pipeline-admin.tsx` (all 6 JSX regions: cards/sections → `bg-surface` + `border-border` + `rounded-xl`; section headings/descriptions → `text-text-primary`/`text-text-secondary`; intro + step labels keep the pre-existing data-driven chips on `bg-*-soft text-*` token families; status/step chips → `Badge`-based chips; inputs → shared `Input`, textarea → shared `Textarea`; raw buttons → shared `Button` (default primary orange for CTAs; `variant="outline"` + `border-danger text-danger` / `border-warning text-warning` destructive-outline treatments); selection checkboxes → `accent-primary`; "Mở danh sách Lesson" `Link` → manual primary-button styling (`bg-primary text-on-primary hover:bg-primary-hover rounded-lg`); all labels, handlers, `id`s (`course-research-topic`), `name`s (`source`, `manualUrl`, `optionalSource`), `aria-describedby`, `role="alert"`, form `onSubmit`s, and button text preserved verbatim) + `content-pipeline-admin.test.tsx` (focused token-guard test appended: asserts shared token classes on card/buttons/badge/inputs and, iterating every rendered element, zero legacy palette prefixes and zero slash-opacity color utilities). Verification: focused vitest **21/21** (incl. new token-guard); `npm run lint` exit 0, `npm run typecheck` exit 0, `npm run build` exit 0 (32 pages), `git diff --check` clean; `git status` shows exactly the 3 T031-owned files modified — no `src/app/api/**`, service, repository, migration, or `.specify/feature.json` changes (diff guard pass). Browser verification (mock `:54321` + dev `:3000`, `E2E Admin` session, real `/admin/content`): computed styles confirm tokens live — main `bg-background` `#fcf9f8` (dark `#171210`), primary buttons `bg-primary` `#a23f00` + `text-on-primary` white, inputs `bg-surface` white / `border-border` `#e8e8e8` (dark surface `#28221d`, border `#544940`, text `#f5efe9`); axe WCAG 2 A/AA **0 violations** at 1440×900 and 390×844 in both light and dark (a `color-contrast` hit immediately after toggling `.dark` was a mid-`transition-colors` artifact — clean on settled re-scan; final matrix all 0); console 0 errors/warnings; behavior smoke on the live page — "Bắt đầu workflow mới" shows the expected "Đã mở workflow mới…" state toast and the "Chủ đề Course" controlled input keeps `id="course-research-topic"` + label association and updates normally. No commit (changes left uncommitted per task convention).

- [x] **T032** [E3] `/moderation` queue + item card restyle
  - **Phase**: E · **Story**: US3 · **[P]**: no · **Prereq**: T023
...
- **T032 record**: `/moderation` queue + item card aligned with the accepted Phase A–C system — visual-only, no functional change. Edits: `src/app/(main)/moderation/page.tsx` (`<main>` → `bg-background py-8 sm:py-10 lg:py-12`; content into `PageContainer pb-16 lg:pb-0` for mobile bottom-nav clearance; metadata untouched) + `src/features/moderation/components/moderation-queue-view.tsx` (header card → shared `PageHeader` — same title/description text (h2→h1); `Tạo Exercise` link → primary-button token styling `bg-primary text-on-primary hover:bg-primary-hover` + `ring-focus-ring`; filter → shared `Select` (`id=status-filter` + label text, onChange + `setPage(1)` reset unchanged); pagination raw `<button>`s → shared `Button variant="outline" size="sm"` with identical onClick/disabled; loading/empty → `bg-surface border-border`; error → `role="alert" border-danger bg-danger-soft text-danger` per T030/T031 convention; spinner `border-t-primary`; text → `text-text-primary/secondary/muted`) + `src/features/moderation/components/moderation-queue-item-card.tsx` (card → `bg-surface border-border`, title/description/footer tokens, links `text-primary hover:text-primary-hover` + focus ring; status chip → shared `Badge`, decorative dot `aria-hidden`; **chip mapping**: the T032 objective says container tokens but the confirmed token set has **no** `warning-container` / `on-success-container`, so all five statuses use the WCAG-verified (T021) B-family soft/base pairs of the app's one chip system (FR-018; Badge variants / T029 / T031) per status hue — pending `warning-soft/warning`, approved `success-soft/success`, rejected `danger-soft/danger`, needs_revision `info-soft/info`, published `primary-soft/primary`, fallback `surface-subtle/text-secondary`; difficulty chips same B pairs; lessonTitle tag `info-soft/info`; exerciseType tag `surface-container`) + NEW `src/features/moderation/components/__tests__/moderation-queue-view.test.tsx` (focused, no snapshots: data + hrefs unchanged; status→token mapping incl. fallback chip; filter refetch URL `status=approved&page=1&limit=10` + page reset; pagination page=2→3 with boundary disabled; loading→error `role="alert"` + 403 permission message; empty state; legacy-palette/dark-prefix/slash-opacity token-guard). Filter/pagination fetch `/api/moderation/generated-exercises`, queue semantics, permissions untouched. Browser verification (mock `:54321` dev `:3000`, admin session, real queue e2e draft 3001): **1440×900 light+dark** axe AA 0 violations/23 passes, 0 horizontal overflow, 2-col grid, header actions single row; **1024×768 light+dark** axe AA 0/23, 0 overflow; **390×844 light+dark** axe AA 0/23, 0 document overflow — a mobile overflow found in this session (34px light / 19px dark) was a PageHeader `shrink-0` actions-slot sizing to 393px max-content vs 358px `PageContainer` content, fixed by wrapping the `Tạo Exercise` CTA + `Trạng thái:` group in `flex flex-col items-start gap-3 sm:flex-row sm:items-center` (actions now stack <sm; desktop single row unchanged), so no shared component was touched; CTA/select touch targets 40px, bottom-nav clearance cardBottom 518 < navTop 775, focus-visible `ring-2 ring-focus-ring ring-offset-background` verified on the CTA link/select/card link/nav via Tab walk; filter smoke live (approved → empty state "Không có bài tập nào khớp với bộ lọc hiện tại.", back to pending → item + "Chờ duyệt" chip return; `status` query + page reset unchanged); console 0 errors / 0 warnings; screenshots `tmp/t032-moderation-{light,dark}-mobile.png` + `tmp/t032-moderation-dark-desktop.png`. **Documented residuals (shared/systemic, out of T032 scope)**: error alert `text-danger` on `danger-soft` = 3.66:1 (< 4.5 AA) is the packet-mandated T030/T031 convention and the app-wide error/danger pair (Badge danger variant/FR-018, T014) — a systemic token-pair property; a `--danger` darkening like T021's `--info` is a token-level/Phase F decision; mobile bottom nav `min-w-20` items (8×80 = 640 > 390) scroll INSIDE the shared `AppNavigation` container (`overflow-x-auto`, T012 mobile bars untouched) and never add document overflow — all pages affected; h1 color inherits pre-existing `text-slate-950` from `(main)/layout.tsx` (T020 deferral, ≈16:1 non-issue). Gates PASS: focused vitest **8/8**, `npm run lint` exit 0, `npm run typecheck` exit 0, `npm run build` exit 0, `git diff --check` clean; `git status` shows only the 4 T032-owned files + new `__tests__` dir modified; changes uncommitted per process, T033+ unchecked.
